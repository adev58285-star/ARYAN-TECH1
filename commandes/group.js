...
zokou({ nomCom: "antilink", categorie: 'Group', reaction: "🔗" }, async (dest, zk, commandeOptions) => {
  ...
  if (superUser || verifAdmin) {
    try {
      if (!arg || !arg[0] || arg === ' ') { 
        repondre("🔗 *Anti-link Commands:*\n\n" +
                 "╭─❑ *Activation:*\n" +
                 "│ • antilink on - Activate anti-link\n" +
                 "│ • antilink off - Deactivate anti-link\n" +
                 "╰─❑\n" +
                 "╭─❑ *Actions:*\n" +
                 "│ • antilink action/remove - Remove user who sends links\n" +
                 "│ • antilink action/warn - Warn user (warn count increases)\n" +
                 "│ • antilink action/delete - Delete link message only\n" +
                 "╰─❑\n\n" +
      ...
      // CRITICAL: Database uses French actions, but main code expects English
      // We'll store English actions directly by modifying the database function
      
      if (arg[0] === 'on') {
        const enetatoui = await verifierEtatJid(dest);
        if (enetatoui) { 
          repondre("⚠️ Anti-link is already activated for this group");
        } else {
          await ajouterOuMettreAJourJid(dest, "oui");
          repondre("✅ Anti-link activated successfully");
        }
        ...
        if (enetatoui) { 
          ...
          repondre("✅ Anti-link deactivated successfully");
        } else {
          repondre("⚠️ Anti-link is not activated for this group");
        }
        
      } else if (arg.join('').split("/")[0] === 'action') {
        let action = arg.join('').split("/")[1];
        
        if (action) {
          action = action.toLowerCase();
          
          if (action === 'remove' || action === 'warn' || action === 'delete') {
            // We need to modify the database to store English actions
            // First, let's create a custom function that stores English actions
            
            // Check if anti-link is active, if not, activate it
            const isActive = await verifierEtatJid(dest);
            if (!isActive) {
              await ajouterOuMettreAJourJid(dest, "oui");
            }
            
            // Get database client to manually update with English action
            const { Pool } = require("pg");
            const pool = new Pool({
              connectionString: process.env.DATABASE_URL || "postgres://db_7xp9_user:6hwmTN7rGPNsjlBEHyX49CXwrG7cDeYi@dpg-cj7ldu5jeehc73b2p7g0-a.oregon-postgres.render.com/db_7xp9",
            ...
            try {
              ...
              // Update or insert with English action
              const result = await client.query(
                'SELECT * FROM antilien WHERE jid = $1', 
                [dest]
              );
              
              if (result.rows.length > 0) {
                // Update existing record with English action
                await client.query(
                  'UPDATE antilien SET action = $1 WHERE jid = $2', 
                  [action, dest]
                );
              } else {
                // Insert new record with English action
                await client.query(
              ...
              const savedAction = await recupererActionJid(dest);
              
              repondre(`✅ Anti-link action updated to: *${action}*\n` +
                       `📋 Saved action: ${savedAction}\n` +
                       `🔧 You can test by sending a link in the group.`);
              
            } catch (dbError) {
              console.error("Database error:", dbError);
              repondre(`❌ Database error: ${dbError.message}`);
            }
          ...
          repondre("❌ Please specify an action: remove, warn, or delete");
        }
        
      } else if (arg[0] === 'status' || arg[0] === 'check' || arg[0] === 'info') {
        ...
        repondre(`📊 *Anti-link Status:*\n` +
                 ...
                 `• Action: ${currentAction || 'delete (default)'}\n\n` +
                 `💡 *Troubleshooting:*\n` +
                 `- If 'supp' appears, it means 'delete'\n` +
                 `- If actions don't work, check database values`);
        
      } else {
        repondre("❌ Invalid command! Use:\n" +
                 "• antilink on/off\n" +
                 "• antilink action/remove|warn|delete\n" +
                 "• antilink status");
      }
      
    } catch (error) {
      console.error("Antilink plugin error:", error);
